...
10. 【严格禁止】所有字段的文案内容中，绝对不得出现具体价格数字（如"199 сомони"、"TJS 50"、"5 сомони"等），也不得出现任何货币单位（сомони、TJS、元、$等），也不得出现"за X сомони"、"всего X"等价格表达句式。允许使用"价格实惠"、"性价比高"、"经济实用"等模糊价值表述，但禁止任何具体金额数字。`;
